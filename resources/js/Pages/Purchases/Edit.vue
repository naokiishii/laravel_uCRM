...
<template>
    <Head title="Edit Purchase History" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Purchase Item</h2>
        </template>
                                ...
                                <div class="container px-5 py-24 mx-auto">
                                    <div class="lg:w-1/2 md:w-2/3 mx-auto">
                                        <div class="flex flex-wrap -m-2">
                                            ...
                                            <div class="p-2 w-full">
                                                <div class="relative">
                                                    <label for="customer" class="leading-7 text-sm text-gray-600">Customer
                                                    </label>
                                            ...
                                            <div class="p-2 w-full">
                                                <div class="">
                                                    <label for="price" class="leading-7 text-sm text-gray-600">Total
                                                        Price</label><br>
                                                    <div 
                                                        class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{{ totalPrice }}JPY</div>
                                                    <InputError class="mt-2" :message="errors.price" />
                                                </div>
                                            </div>
                                            <div class="p-2 w-full">
                                                <div class="relative">
                                                    <label for="status" class="leading-7 text-sm text-gray-600">Status
                                                    </label>
                                                    <input disabled type="radio" id="status" v-model="form.status" name="status" :value="1">Not Cancelled
                                                    <input disabled type="radio" id="status" v-model="form.status" name="status" :value="0">Cancelled
                                                    <InputError class="mt-2" :message="errors.name" />
                                                </div>
                                            </div>
